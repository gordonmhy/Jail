...
class ResetmineListener extends BaseListener
{
    ...
    public function onSignChange(SignChangeEvent $event)
    {
        ...
        if ($mines->hasMineSet($ja) !== true) {
            $event->getBlock()->getLevel()->setBlock(new Vector3($event->getBlock()->x, $event->getBlock()->y, $event->getBlock()->z), Block::get(0));
            $event->getPlayer()->sendMessage($this->getPlugin()->getMessage("mine.not.set"));
            return false;
        }
        $event->setLine(0, $this->getPlugin()->colorMessage("&7[" . $this->getPlugin()->getMessage("sign.resetmine.tag") . "&7]"));
    ...
    public function onPlayerInteract(PlayerInteractEvent $event)
    {
        ...
        if ($sign[0] == $this->getPlugin()->colorMessage("&7[" . $this->getPlugin()->getMessage("sign.resetmine.tag") . "&7]")) {
            ...
            $this->getPlugin()->getServer()->dispatchCommand($event->getPlayer(), "jailresetmine");
            return true;
        }
    }
