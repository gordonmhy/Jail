...
class Economyapi extends BaseEconomy
{
    ...
    public function bail(Player $player): bool
    {
        ...
        $player->sendMessage(str_replace("%deduction%", ($this->getPlugin()->prisoner_time[strtolower($player->getName())] * ($this->getPlugin()->getConfig()->get("bail-per-second")) + 1), str_replace("%remaining%", $this->getPlugin()->getEco()->getInstance()->myMoney($player), $this->getPlugin()->getMessage("bail.money.remaining"))));
