    ...
    public function onCommand(CommandSender $issuer, Command $cmd, $label, array $args)
    {
        switch ($cmd->getName()) {
            case "votejail":
                ...
                if (in_array(strtolower(strtolower($issuer->getName())), $t[strtolower($target->getName())]["VoteForJail"]["votedBy"]) !== false) {
                    $issuer->sendMessage($this->getPlugin()->getMessage("vote.sender.votedAlready"));
                    return true;
                }
                if ($this->getPlugin()->voteForJail(strtolower($target->getName()), strtolower(strtolower($issuer->getName()))) !== false) {
