        ...
        if ($this->getPlugin()->getConfig()->get("show-prisoner-timer") !== false) {
            foreach ($this->getPlugin()->getAllJailedPlayerNames() as $name) {
                ...
                if ($player !== null) {
                    ...
                    $bail_dis = ("&2" . $this->getPlugin()->getMessage("timer.broadcast.bail") . ": &d") . (($this->getPlugin()->getEco() !== null && $this->getPlugin()->getConfig()->get("allow-bail") !== false && filter_var($j[$t[$player->getName()]["jail"]]["allow-bail"], FILTER_VALIDATE_BOOLEAN) !== false) ? ($this->getPlugin()->isJailTimeInfinite(strtolower($player->getName())) !== false) ? $this->getPlugin()->getMessage("timer.broadcast.notallowed") : "$" . ($this->getPlugin()->prisoner_time[strtolower($player->getName())] * ($this->getPlugin()->getConfig()->get("bail-per-second"))) : ($this->getPlugin()->getMessage("timer.broadcast.disabled") . ($this->emoji < 8 ? " ^o^" : " ")));
